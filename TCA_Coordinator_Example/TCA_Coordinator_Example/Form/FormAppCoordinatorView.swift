...
struct FormAppCoordinator: Reducer {
    struct State: IdentifiedRouterState, Equatable {
        static let initalState = Self(routeIDs: [.root(.step1, embedInNavigationView: true)])
        ...
        var finalScreenState: FinalScreen.State {
            return .init(
                ...
                dateOfBirth: step2State.dateOfBirth,
                job: step3State.selectedOccupation
            )
        }
        ...
        var routes: IdentifiedArrayOf<Route<FormScreen.State>> {
            get {
                let routes = routeIDs.map { route -> Route<FormScreen.State> in
                    route.map { id in
                        switch id {
                        case .step1: return .step1(step1State)
                        case .step2: return .step2(step2State)
                        case .step3: return .step3(step3State)
                        case .finalScreen: return .finalScreen(finalScreenState)
                        }
                    }
                }
                ...
                let routeIDs = newValue.map { route -> Route<FormScreen.State.ID> in
                    route.map { id in
                        ...
                        case let .step1(state):
                            self.step1State = state
                            return .step1
                            
                        case let .step2(state):
                            self.step2State = state
                            return .step2
                            
                        case let .step3(state):
                            self.step3State = state
                            return .step3
    ...
    var body: some View {
        TCARouter(store) { screen in
            SwitchStore(screen) { screen in
                switch screen {
                case .step1:
                    CaseLet(/FormScreen.State.step1, action: FormScreen.Action.step1, then: Step1View.init)
                case .step2:
                    CaseLet(/FormScreen.State.step2, action: FormScreen.Action.step2, then: Step2View.init)
                case .step3:
                    CaseLet(/FormScreen.State.step3, action: FormScreen.Action.step3, then: Step3View.init)
                case .finalScreen:
                    CaseLet(/FormScreen.State.finalScreen, action: FormScreen.Action.finalScreen, then: FinalScreenView.init)
                }
            }
        }
    }
